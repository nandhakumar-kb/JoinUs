class FormHandler {
  constructor() {
    this.storageKey = 'joinus_feedback_submissions';
    this.init();
  }
  ...
  sendEmailNotification(submission) {
    const subject = encodeURIComponent('New JoinUs Feedback Submission');
    ...
    submission.mailtoLink = `mailto:your-email@example.com?subject=${subject}&body=${body}`;
  }
  ...
  showAdminPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="panel-header" style="justify-content: flex-end;">
        <button class="export-btn" onclick="formHandler.exportSubmissions()">
          📥 Export All (CSV)
        </button>
      </div>
      ...
      <div id="submissions-list">
        ${submissions.length === 0 ? 
          ...
          submissions.map(sub => this.renderSubmission(sub)).join('')
        }
      </div>
    `;
    ...
    return `
      <div class="submission" data-id="${sub.id}">
        ...
        <div class="action-btns">
          <button class="email-btn" onclick="formHandler.replyViaEmail('${sub.id}')">
            ✉️ Reply via Email
          </button>
          <button class="delete-btn" onclick="formHandler.deleteSubmission('${sub.id}')">
            🗑️ Delete
          </button>
        </div>
      </div>
